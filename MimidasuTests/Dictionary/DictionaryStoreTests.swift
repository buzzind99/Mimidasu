...
func makeJMDictSmokeDatabase(includeSmokeWord: Bool = true) throws -> URL {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("mimidasu-jmdict-smoke-\(UUID().uuidString)", isDirectory: true)
...
final class DictionaryStoreTests {
    ...
    init() throws {
        tempRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("mimidasu-dictstore-\(UUID().uuidString)", isDirectory: true)
    ...
    /// Repo root (MimidasuTests/Dictionary/ → repo), for the script-fetched real
    /// model.
    ...
    func envOverrideWins() {
        ...
        let url = DictionaryStore.resolve(
            environment: ["MIMIDASU_DICT": overridePath], fileExists: overrideExists
        )
    ...
    func missingEnvOverrideFallsThrough() {
        ...
        let url = DictionaryStore.resolve(
            environment: ["MIMIDASU_DICT": missingOverride], fileExists: dictionariesExist
        )
    ...
    #if DEBUG
        @Test("resolves the MIMIDASU_DICT override set on the process")

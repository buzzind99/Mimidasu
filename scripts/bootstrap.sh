...
# CODE_SIGN_IDENTITY (project.yml does not set it, so the xcconfig wins) and
# of CURRENT_PROJECT_VERSION — the commit count, so dev builds carry a real
# CFBundleVersion. project.yml deliberately does not set that key either.
# Packaging scripts (package.sh / package_mas.sh) compute their own build
# number and pass it on the xcodebuild command line.
...
# 3. Write the xcconfig the generated project reads its identity and build
#    number from. The commit count is monotonic without a state file; the
#    snapshot refreshes on each run (re-run with --skip-generate after
#    committing to bump dev build numbers).
...
echo "Build number (CURRENT_PROJECT_VERSION): ${build_number}"
if [[ "${GENERATE}" -eq 1 ]]; then

#!/usr/bin/env bash
#
# Build the tokenizer dylib (one-time, dev machine), stage it into the repo
# so the app can load it, and fetch the pinned IPADIC model.
#
#   scripts/build_tokenizer.sh
#
# Outputs:
#   local/frameworks/libdictionary.dylib    FFI dylib (tokenizer core compiled
#                                           in; no companion dylibs) — the
#                                           filename is the app-facing FFI
#                                           name, not a statement that this
#                                           script builds the JMDict DB
#                                           (that's build_dictionary.sh)

...
enum ModelVerifier {
    ...
    static let checksumMismatchMessage =
        "model file does not match Mimidasu's pinned checksum — "
    ...
    final class VerdictStore: Sendable {
        /// Production location: `~/Library/Application Support/Mimidasu/
        /// verification-cache.json` (sibling of the models directory).
        static let sharedURL: URL = {
            let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            return base.appendingPathComponent("Mimidasu/verification-cache.json")
        }()

...
///
/// A global `CATapDescription` tap (excluding Mimidasu's own process) is attached
...
final class SystemAudioCapture: NSObject, AudioCapturing, @unchecked Sendable {
    ...
    private let ioQueue = DispatchQueue(
        label: "mimidasu.capture.tap", qos: .userInteractive
    )
    private let listenerQueue = DispatchQueue(
        label: "mimidasu.capture.tap.listener", qos: .userInitiated
    )
    ...
    func start() async throws {
        ...
        do {
            // Whole-system mix minus Mimidasu itself.
    ...
    ///
    /// The SDK annotates the input-block parameter `NS_NOESCAPE` and
    /// non-`@Sendable`, so the block cannot escape `convert` and is
    /// invoked serially on this IO thread; its single feed is gated by
    /// a plain captured flag — no synchronization needed. The input
